...
import time
from shot_im_utils import *

log = logging.getLogger("app")

class SHOT_IM(pl.LightningModule):
    def __init__(
        self,
        arch: str = "Resnet18",
        ...
        self.netF, self.optimizer_netF = get_model(arch, data_dir, dataset, 2 + self.num_classes, pretrained= pretrained, \
        ...
        self.netB = feat_bottleneck(type='bn', feature_dim=self.netF.in_features, bottleneck_dim=256).cuda()
        self.netC = feat_classifier(type='wn', class_num = num_source_classes, bottleneck_dim=256).cuda()
        param_group = []
        ...
        self.model_path = "/export/r36a/data/schaud35/shiftpu/models/"
